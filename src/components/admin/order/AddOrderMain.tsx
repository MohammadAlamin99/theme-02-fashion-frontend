...
//       const pData = itemProductQueries[index]?.data;
//       return {
//         id: item.productId,
...
//   // Keep shippingArea state updated if current selection is invalid
//   React.useEffect(() => {
...
//       const fullProd = singleP || product;
...
//   const addItem = (product: Product, selectedVariant: any | null) => {
...
export default function AddOrderMain() {
  ...
  const handleSelectProductFromSearch = async (product: Product) => {
    try {
      const singleP = await fetchSingleProduct(product.id);
      const fullProd = (singleP as Product) || product;
      if (fullProd.variants && fullProd.variants.length > 0) {
        setSelectedProductForVariant(fullProd);
      } else {
        addItem(fullProd, null);
