...
//       // Step 1: backend দেওয়া real key দিয়ে সরাসরি match
//       const directMatch = dynamicShippingOptions.find(
//         (opt) => opt.key === shipping.shippingArea,
//       );
//       if (directMatch) {
//         userSelectedShippingRef.current = true;
//         return; // already correct, kichu korar dorkar nai
//       }

//       // Step 2: fee দিয়ে tolerant match (১ টাকা পর্যন্ত পার্থক্য মেনে নেওয়া)
//       const feeMatch = dynamicShippingOptions.find(
//         (opt) =>
//           Math.abs(Number(opt.fee) - Number(shipping.actualShippingFee)) < 1,
//       );
//       if (feeMatch) {
//         userSelectedShippingRef.current = true;
//         setShipping((prev) => ({ ...prev, shippingArea: feeMatch.key }));
//         return;
//       }

//       // Step 3: কোনো match-ই না পেলে — ভুল option silently select না করে
//       // আসল stored fee-টাই একটা "Saved" option হিসেবে দেখানো
//       userSelectedShippingRef.current = true;
//       setShipping((prev) => ({ ...prev, shippingArea: "__saved__" }));
//       return;
//     }
...
//   const dynamicShippingOptionsWithSaved = useMemo(() => {
//     if (
//       isEditMode &&
//       shipping.shippingArea === "__saved__" &&
//       shipping.actualShippingFee !== null
//     ) {
//       return [
//         {
//           key: "__saved__",
//           label: "Saved delivery charge",
//           fee: shipping.actualShippingFee,
//         },
//         ...dynamicShippingOptions,
//       ];
//     }
//     return dynamicShippingOptions;
//   }, [
//     dynamicShippingOptions,
//     isEditMode,
//     shipping.shippingArea,
...
//     setShipping({
//       shippingArea: existingOrder.shipping_area || "",
...
//                     onChange={(e) => {
//                       userSelectedShippingRef.current = true;
//                       if (e.target.value === "__saved__") {
//                         // "Saved delivery charge" select korle actual fee keep korbo
//                         return;
//                       }
//                       setShipping((prev) => ({
//                         ...prev,
...
//                       </option>
//                     ) : (
//                       dynamicShippingOptionsWithSaved.map((opt) => (
//                         <option key={opt.key} value={opt.key}>
//                           {opt.label}
//                           {typeof opt.fee === "number" && opt.fee > 0
//                             ? ` (৳${opt.fee})`
//                             : ""}
//                         </option>
//                       ))
...
type OrderItemFromApi = {
  ...
  variant?: { id?: string; images?: string[] } | null;
};
...
export default function AddOrderMain() {
  ...
  const zoneOptions = useMemo(() => {
    return buildZoneShippingOptions(shippingSettings?.courier_config);
  }, [shippingSettings]);
  ...
  const calculatedShippingFee = useMemo(() => {
    if (
      ...
      shipping.actualShippingFee !== null &&
      // eslint-disable-next-line react-hooks/refs
      !userSelectedShippingRef.current
    ) {
      return shipping.actualShippingFee;
    }
    const selectedOpt = zoneOptions.find(
      (opt) => opt.key === shipping.selectedShippingKey,
    );
    if (selectedOpt) {
      const { totalShippingFee } = calculateCartShippingDetails(
        cartItemsWithShipping,
        selectedOpt.shippingArea,
        shippingSettings,
        selectedOpt.fee,
      );
  ...
  // Effect to match the shipping key in Edit mode
  useEffect(() => {
    if (
      isEditMode &&
      shipping.actualShippingFee !== null &&
      ...
      if (match) {
        ...
        setShipping((prev) => ({
          ...prev,
          selectedShippingKey: match.key,
          shippingArea: match.shippingArea,
        }));
      } else {
        setShipping((prev) => ({ ...prev, selectedShippingKey: "__saved__" }));
      }
    }
  }, [zoneOptions, isEditMode, shipping.actualShippingFee]);

  const subtotal = items.reduce(
    (acc, item) => acc + item.sell_price * item.quantity,
    0,
  );
  const totalDue = subtotal + calculatedShippingFee - shipping.manualDiscount;
  const remainingDue = totalDue - shipping.advanceAmount;
  ...
  const handleSelectProductFromSearch = async (product: Product) => {
    try {
      ...
      const fullProd: Product = singleP
        ? {
            ...singleP,
            images: singleP.images?.map((img: { url: string }) =>
              typeof img === "string" ? img : img.url,
            ),
          }
        : product;
  ...
  return (
    <div className="bg-[#F9FAFB] min-h-screen font-lato">
      <div className="">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    value={shipping.selectedShippingKey}
                    onChange={(e) => {
                      const key = e.target.value;
                      const opt = zoneOptions.find((o) => o.key === key);
                      userSelectedShippingRef.current = true;
                      setShipping((prev) => ({
                        ...prev,
                        selectedShippingKey: key,
                        shippingArea: opt ? opt.shippingArea : "outside",
                        actualShippingFee: null,
                      }));
                    }}
                    className="w-full p-2.5 bg-gray-100 rounded-lg text-sm mt-2 font-semibold cursor-pointer"
                  >
                    <option value="">Select Shipping Location</option>

                    {/* Step 1 Fix: unique key using zone.zone or index */}
                    {shippingSettings?.courier_config?.zones?.map(
                      (zone: any, index: number) => (
                        <optgroup
                          key={
                            zone.id ? `zone-${zone.id}` : `zone-idx-${index}`
                          }
                          label={zone.zone}
                          className="text-blue-600"
                        >
                          {zoneOptions
                            .filter((opt) => opt.zoneName === zone.zone)
                            .map((opt) => (
                              <option
                                key={opt.key}
                                value={opt.key}
                    ...
                    {isEditMode && shipping.actualShippingFee !== null && (
                      <option value="__saved__">
                        Saved Fee (৳{shipping.actualShippingFee})
                      </option>
                    )}
                  </select>
                </div>
            ...
            {/* Financials - KEEPING YOUR ORIGINAL DESIGN (Red/Green Inputs) */}
            <div className="bg-white p-5 rounded-xl shadow-sm">
              <div className="flex items-center gap-2 mb-4 text-[#1DA1F2]">
                <CreditCard size={20} />
                <h2 className="text-lg font-medium text-gray-800">
      ...
      {/* Variant Modal - KEEPING YOUR ORIGINAL DESIGN */}
      {selectedProductForVariant && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[10002] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6 text-left">
            <div className="flex justify-between items-center pb-4 border-b border-gray-100">
            ...
            <div className="mt-4 space-y-3 max-h-[350px] overflow-y-auto pr-1">
              {selectedProductForVariant.variants?.map((v) => (
                <div
                  key={v.id}
                  onClick={() => addItem(selectedProductForVariant, v)}
                  className="p-3 border rounded-xl hover:border-[#1DA1F2] hover:bg-blue-50/50 cursor-pointer flex items-center justify-between transition-all"
                >
                  <div className="flex items-center gap-3">
                    <Image
