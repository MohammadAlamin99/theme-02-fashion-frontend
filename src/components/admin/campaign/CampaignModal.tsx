...
export default function CampaignModal({ mode, data, onClose }: ModalProps) {
  ...
  const handleImageUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res: any = await uploadCampaignImage(file);
      const imageUrl =
        res?.image_url || res?.data?.image_url || res?.url || res?.data?.url;
        ...
        <div className="p-6 max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <Image
                      src={
                        formData.image.startsWith("http")
                          ? formData.image
                          : `${backendBaseUrl}/${formData.image.replace(/^\/+/, "")}`
                      }
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button
                disabled={mutation.isPending || uploading || uploadingImage}
                type="submit"
