    ...
    <div>
      ...
      >
        <div className="flex items-center gap-3">
          <Icon
            size={20}
            className={isOpen ? "text-[#38BDF8]" : "text-gray-500"}
          />
          <span
            className={`text-base font-normal ${isOpen ? "text-[#38BDF8]" : "text-black"}`}
          >
            {item.label}
          </span>
        </div>
          ...
          {item.submenu?.map((sub) => {
            ...
            return (
              <Link key={sub.href} href={sub.href} className="block">
                <div
                  className={`flex items-center gap-3 pl-12 py-2 mx-2 rounded-[5px] transition-colors
                                    ${isSubActive ? "text-[#38BDF8]" : "text-black"}`}
                >
                  <ChevronRight
                    size={24}
                    className={isSubActive ? "text-[#38BDF8]" : "text-black"}
                  />
