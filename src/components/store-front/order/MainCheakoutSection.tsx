...
const MainCheckoutSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (stored) {
      setTimeout(() => setOrderSource(stored), 0);
    }
  }, []);
  ...
  const activeCampaigns = useMemo(() => {
    const raw = Array.isArray(activeCampaignsData?.data)
      ? activeCampaignsData.data
      : Array.isArray(activeCampaignsData)
        ? activeCampaignsData
        : (activeCampaignsData as unknown as { data: { data: Product[] } })
            ?.data?.data || [];
  ...
  const cartItems: CartItem[] = useMemo(() => {
    return rawCartItems.map((item, index) => {
      ...
      const displayBasePrice =
        parseFloat(String(variantPrice)) ||
        parseFloat(String(item.sell_price)) ||
        productBasePrice;
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => applyCouponMutation.mutate(couponInput.trim())}
                disabled={applyCouponMutation.isPending || !couponInput.trim()}
                className={`px-8 py-3.5 rounded-xl text-white transition-colors ${
                  couponInput.trim()
                    ? "bg-[#103160] cursor-pointer"
