...
const ChatWidget = () => {
  ...
  const renderAttachmentPreview = (att: Attachment, isMe: boolean) => {
    ...
    return (
      ...
      >
        <FiFileText
          size={16}
          className={isMe ? "text-white" : "text-[#000000]"}
        />
  ...
  return (
    <div className="fixed bottom-[85px] right-4 lg:bottom-6 lg:right-6 z-[210] font-sans flex flex-col items-end antialiased selection:bg-[#000000]">
        ...
        <div className="flex flex-col gap-3 mb-4 animate-in fade-in slide-in-from-bottom-5 duration-300 relative items-end">
              ...
              <div className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-gray-100 mb-4 group transition-all hover:bg-slate-100">
                ...
                {settings?.phone && (
                  <button
                    ...
                    className="text-[#000000] border-none bg-transparent cursor-pointer p-1"
                  >
                    {copied ? (
          ...
          {enableLiveChat && (
            <button
              ...
              className="relative flex items-center justify-center w-12 h-12 bg-[#000000] text-white rounded-full shadow-lg hover:scale-110 transition-all border-none cursor-pointer"
              title="Live Chat"
            >
              <FiMessageSquare size={22} />
              {/* Badge on the inner option button */}
              {unreadMessageCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-[#000000] text-white text-[10px] font-bold h-5 w-5 rounded-full flex items-center justify-center border-2 border-white">
                  {unreadMessageCount}
                </span>
              )}
            </button>
      ...
      {isOpen && (
        <div className="w-[calc(100vw-32px)] sm:w-[400px] h-[480px] sm:h-[520px] bg-white border border-gray-100 rounded-2xl shadow-2xl flex flex-col overflow-hidden mb-4 transform origin-bottom-right animate-in fade-in zoom-in-95 duration-200">
          {/* Header */}
          <div className="p-4 bg-gradient-to-r from-[#000000] to-[#000000] text-white flex items-center justify-between shrink-0 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center font-bold text-sm relative border border-white/10 shadow-inner">
                OP
                <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-400 border-2 border-[#000000] rounded-full shadow-sm animate-pulse" />
              </div>
              <div>
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-[#F8FAFC] space-y-3.5 custom-scrollbar">
            {loadingHistory ? (
              <div className="h-full flex flex-col items-center justify-center text-gray-400 gap-2">
                <FiLoader className="animate-spin text-[#000000]" size={20} />
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {msg.text && (
                        <div
                          className={`p-3 text-[13px] leading-relaxed shadow-3xs border ${isMe ? "bg-[#000000] text-white rounded-2xl rounded-tr-none border-transparent font-medium" : "bg-white text-gray-800 rounded-2xl rounded-tl-none border-gray-200/60 font-normal"}`}
                        >
                          {msg.text}
                        </div>
                      )}
          ...
          <div className="p-3 border-t border-gray-100 bg-white shrink-0 flex flex-col gap-2">
                  ...
                  return (
                    ...
                    >
                      {att.type === "IMAGE" ? (
                        ...
                        <FiFileText size={16} className="text-[#000000] ml-1" />
                      )}
            ...
            <form
              ...
              className="flex items-center gap-2.5 bg-slate-50 border border-gray-200 rounded-xl px-3.5 py-2.5 focus-within:bg-white focus-within:border-[#000000] transition-all relative"
            >
              <input
                type="file"
                ref={fileInputRef}
              ...
              {/* <button
                ...
                className="text-gray-400 hover:text-[#000000] transition-colors cursor-pointer border-none bg-transparent p-0 flex items-center"
              >
                {uploading ? (
                  <FiLoader className="animate-spin text-[#000000]" size={18} />
                ) : (
                  <FiPaperclip size={18} />
                )}
              </button> */}
              ...
              <button
                ...
                className="text-[#000000] bg-transparent border-none cursor-pointer flex items-center disabled:opacity-30"
              >
                <FiSend size={18} />
              </button>
            </form>
      ...
      <button
        ...
        type="button"
        className="relative bg-[#000000] text-white w-14 h-14 rounded-full hover:bg-[#000000] transition-all duration-300 hover:scale-110 active:scale-95 cursor-pointer flex items-center justify-center z-[100] border-none outline-none"
      >
        {isOpen || showOptions ? (
          <FiX size={28} />
        ) : (
          <>
            ...
            {/* 🚀 THE MAIN RED BADGE */}
            {unreadMessageCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#000000] text-white font-bold text-[10px] min-w-[20px] h-5 px-1 rounded-full flex items-center justify-center border-2 border-white shadow-md animate-in zoom-in">
                {unreadMessageCount}
              </span>
            )}
          </>
