...
export default async function LegalPage({
  ...
  if (!page) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        {language === "BAN" ? "পৃষ্ঠা পাওয়া যায়নি" : "Page not found"}
      </div>
    );
  }
    ...
    <main className="py-10 md:py-16">
      <div className="max-w-[1720px] mx-auto px-6">
        ...
        <nav className="flex items-center text-sm text-[#727272] mb-6 font-medium">
          ...
          <ChevronRight size={16} className="mx-2" />
          <span className="text-[#7CB640]">{page.title}</span>
        </nav>

        {/* Content Card */}
        <div className="bg-white p-4 md:p-0 pt-4 rounded-lg relative overflow-hidden">
          <div className="text-[#4A4A4A]">
            {page.content ? (
              <div
                className="prose prose-lg prose-slate max-w-none 
                prose-headings:text-[#003032] prose-headings:font-bold
                prose-p:leading-loose prose-p:text-[17px]
                prose-strong:text-[#003032] prose-strong:font-bold
